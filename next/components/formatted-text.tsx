    ...
    switch (domNode.name) {
      ...
      case "a": {
        ...
        if (href && isRelative(href)) {
          return (
            <Link href={href} className="underline text-primary-500">
              {domToReact(domNode.children, options)}
            </Link>
          );
        } else if (href) {
          return (
            <Link href={href} target="_blank" className="underline text-primary-500 inline-flex gap-1 items-center">
              {domToReact(domNode.children, options)}
      ...
      case "p": {
        return <p className="mb-2 text-md font-regular">{domToReact(domNode.children, options)}</p>;
      }
