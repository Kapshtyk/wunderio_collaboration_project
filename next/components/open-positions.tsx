...
const OpenPositions = ({ openPositions }: OpenPositionsProps) => {
  ...
  useMemo(() => {
    const offices = openPositions.map((openPosition) => openPosition.field_office.name)
    setOffices(offices)
  }, [openPositions])

  useMemo(() => {
    const countries = openPositions.map((openPosition) => openPosition.field_country.name)
  ...
  const filteredPositions = openPositions.filter((openPosition) => {
    ...
    if (choosenOffice === "all" && choosenCountry !== "all") return openPosition.field_country.name === choosenCountry
    if (choosenOffice !== "all" && choosenCountry === "all") return openPosition.field_office.name === choosenOffice
    return openPosition.field_office.name === choosenOffice && openPosition.field_country.name === choosenCountry
  })
        ...
        {filteredPositions.map((openPosition) => (
          ...
          >
            <div className="p-4 w-52 h-52 rounded-md shadow-sm bg-primary-50">
              ...
              <span>{openPosition.field_country.name}, {openPosition.field_office.name}</span>
            </div>
          </Link>
        ))
        }
