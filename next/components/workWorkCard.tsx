import Link from "next/link";
import { MediaImage } from "./media--image";
import { Paragraph, WorkCard } from "@/lib/zod/paragraph";

export function WorkWorkCard({ workPage }) {
    return (
        <div
            key={workPage.id}
            className="block max-w-[16rem] rounded-lg bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700">

            {workPage.field_content_elements.filter((element: Paragraph) => element.type === "paragraph--work_card").map((paragraph: WorkCard) => (
                <div className="relative overflow-hidden bg-cover bg-no-repeat h-64">
                    <Link href={"/work" + workPage.path.alias}>
                        <MediaImage className="rounded-sm h-60 hover:saturate-150" media={paragraph.field_image} />
                    </Link>
                </div>
            ))}

            <div className="p-6">
                <p className="text-base text-sm text-secondary-900">
                    <Link href="/work">
                        <span className="hover:underline">
                            {workPage.field_page_types.name.toUpperCase()}
                        </span>
                    </Link>
                </p>
            </div>

            <Link href={"/work" + workPage.path.alias}>
                <div className="p-6">
                    <h1 className="text-primary-600 font-bold hover:underline">
                        {workPage.title}
                    </h1>
                </div>
            </Link>

        </div>
    )
}